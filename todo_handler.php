...
$sql = "SELECT count(*) FROM Todos"; # query to get count of todos
...
// insert new todo 
if (isset($_POST['todo'])) {
	$sql = "INSERT INTO Todos (description)
	VALUES ('".$_POST['todo']."')"; # Create query to insert new todo from POST request
...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
	if ($count > 0) {
		...
		$sql = "SELECT * FROM Todos"; # Create query to get all current todos
		if (!($result = $conn->query($sql))) {
			echo $sql;
			echo "Query failed: ( $conn->errno ) $conn->error";
		} else {
